...
def PFA_subbytes(matrix):
    size_row = len(matrix)
    size_col = len(matrix[0])
    assert size_row == 4 and size_col == 4
    for i in range(size_row):
        for j in range(size_col):
            matrix[i][j] = PFA_s_box[matrix[i][j]]


def PFA_encrypt(plaintext_matrix, key_matrix, *Table):
    ...
    ##Round 1-9
    for i in range(1,10):
        PFA_subbytes(plaintext_matrix)
        if (i == 9):
            if Table != ():
                Table2 = Table[0]
                round9matrix = [[0 for l in range(0, 4)] for j in range(0, 4)]
                for n in range(0, 4):
                    for m in range(0, 4):
                        round9matrix[n][m] = plaintext_matrix[n][m]
                Table2.append(round9matrix)
